-- Compra em lote para produtos de revenda e insumos reais.
-- Não substitui a RPC legada registrar_compra_lote, usada pela versão já publicada.
-- Aplique depois de 20260831193000_operacoes_cmv_insumos.sql.

create or replace function public.registrar_compra_lote_insumos(
  p_lote_id text,
...
begin
  ...
  if p_fornecedor_id is not null and not exists (select 1 from public.fornecedores where id = p_fornecedor_id) then raise exception 'Fornecedor não encontrado'; end if;
  if p_valor_prazo > 0 and p_data_vencimento is null then raise exception 'Informe a data de vencimento'; end if;
  if exists (select 1 from public.movimentacoes_estoque where motivo = 'Abastecimento via Lote [LOTE-' || p_lote_id || ']') then raise exception 'Este lote já foi registrado'; end if;
  ...
  if round(v_total_imediato + p_valor_prazo, 2) <> round(p_total, 2) then raise exception 'Os pagamentos devem totalizar o valor do lote'; end if;
  if v_dinheiro > 0 and (p_caixa_id is null or not exists (select 1 from public.controle_caixa where id = p_caixa_id and status = 'aberto')) then raise exception 'É necessário um caixa aberto para pagamento em dinheiro'; end if;
  ...
  loop
    ...
    if v_tipo_item not in ('produto', 'insumo') or v_quantidade <= 0 or v_preco_custo < 0 or v_valor_linha <= 0 then raise exception 'Item de lote inválido'; end if;
    v_total_itens := v_total_itens + v_valor_linha;

    if v_tipo_item = 'produto' then
      select * into v_produto from public.produtos where id = (v_item ->> 'produto_id')::uuid and tipo = 'venda' for update;
      if not found then raise exception 'Produto de revenda não encontrado'; end if;
      ...
      select * into v_insumo from public.insumos where id = (v_item ->> 'insumo_id')::uuid for update;
      if not found then raise exception 'Insumo não encontrado'; end if;
      update public.insumos
        set quantidade_estoque = coalesce(quantidade_estoque, 0) + v_quantidade,
            custo_unitario = v_preco_custo,
            preco_total_pago = v_valor_linha
        where id = v_insumo.id;
      insert into public.movimentacoes_estoque (insumo_id, quantidade, tipo_movimento, motivo, atendente)
        values (v_insumo.id, v_quantidade, 'Entrada - Compra Lote', 'Abastecimento via Lote [LOTE-' || p_lote_id || ']', p_atendente);
    end if;
  end loop;
  ...
  if v_total_imediato > 0 then
    select string_agg(distinct value ->> 'metodo', ' + ' order by value ->> 'metodo') into v_metodos from jsonb_array_elements(p_pagamentos_imediatos);
